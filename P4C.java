...
public final class P4C {
    ...
    static void addEdge(int vertID, int endID, Distance<Pixel> pd) {
        ...
        // System.err.println(pixVert1.data() + " " + 
        //pixVert2.data() + " " + dist);
        ...
        while (addEdges < vert && !pQueue.isEmpty()) {
            ...
            if (p.partitionCount() != uCal.size()) {
                throw new IllegalStateException(
                        "Partition count not in sync with UnionConditionsCalcs."
                        + " Partition says: " + p.partitionCount() 
                        + ", uCal says: " + uCal.size() + ": i = " +  i++);
            }
        ...
        try {
          // the line that reads the image file
            ...
            for (Integer id : compIDs) {
                System.err.print("On component: " 
                        + compIdx + " with head at " + verts.get(id).data() 
                        + " of size: " + uCal.sizeOfGroup(id));
                ...
                List<GVertex<Pixel>> componentVertices = 
                        limitedEdgesGraph.depthFirst(verts.get(id));


                System.err.println(" with actual numVerts: " 
                        + componentVertices.size());

                if (uCal.sizeOfGroup(id) == 1) {
                    System.err.println(limitedEdgesGraph.incidentEdges(
                            verts.get(id)));
                }
